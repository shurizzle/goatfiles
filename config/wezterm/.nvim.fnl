    ...
    (if ok
        ...
        (do
          (set package.preload.fennel
               (fn [& args]
                 (let [path (assert (vim.loop.fs_realpath (path-join *project*
                                                                     ".." ".."
                                                                     :fennel.lua))
...
(dfn 250 compile-exrc [] (-> (slurp *fnl-exrc*)
                             (fennel.compile-string {:filename *fnl-exrc*
                                                     :globals [:vim]
                                                     :use-bit-lib true})
                             (spit *lua-exrc*)))

(dfn 250 compile-wezterm [] (-> (slurp *in*)
...
(fn file-changed [err filename]
  (if err
      (vim.api.nvim_echo [[(if (= :string (type err)) err (tostring err))
                           :ErrorMsg]] true [])
      (= filename :.nvim.fnl)
      (compile-exrc)
      (and (not= filename :.nvim.lua) (not= filename :compile.lua)
           (= (filename:sub -4) :.fnl))
      (compile-wezterm)))

(let [handle (uv.new_fs_event)]
  (uv.fs_event_start handle *project*
                     {:recursive true :watch_entry true :stat true} file-changed)
