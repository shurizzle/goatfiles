...
(fennel.install)
(set _G.unpack (or table.unpack _G.unpack))
(tset _G (fennel.mangle :pattern-quote)
      (let [pat (.. "([" (: "%^$().[]*+-?" :gsub "(.)" "%%%1") "])")]
        (lambda [str]
          (str:gsub pat "%%%1"))))
...
(let [sep (package.config:sub 1 1)
  ...
  (set fennel.macro-path (.. fennel.macro-path ";" dir sep "?" sep :init.fnl)) ; XXX: do NOT use async versions in "fs" module
